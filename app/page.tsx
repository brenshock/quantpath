    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
        <div className="flex flex-col justify-between gap-4 border-b pb-6 sm:flex-row sm:items-end">
          <div><h1 className="text-3xl font-semibold tracking-tight">QuantPath curriculum</h1><p className="mt-2 max-w-2xl text-muted-foreground">Learn techniques directly, then test whether you can recognize them in mixed interview-style problems.</p></div>
          <span className="text-sm text-muted-foreground">{problems.length} problems · {learningCategories.length} categories</span>
        </div>
        ...
        {view === 'practice' && <section className="mt-6 rounded-xl border bg-card p-4" aria-label="Find and sort problems">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(15rem,1fr)_repeat(4,minmax(10rem,auto))]">
            ...
            <div><label className="sr-only" htmlFor="practice-category">Problem category</label><NativeSelect id="practice-category" className="w-full" value={practiceCategory} onChange={(event) => setPracticeCategory(event.target.value)}><NativeSelectOption value="all">All categories</NativeSelectOption>{learningCategories.map((item) => <NativeSelectOption key={item.id} value={item.id}>{item.title}</NativeSelectOption>)}</NativeSelect></div>
            <div><label className="sr-only" htmlFor="practice-employer">Employer</label><NativeSelect id="practice-employer" className="w-full" value={employer} onChange={(event) => setEmployer(event.target.value)}><NativeSelectOption value="all">All employers</NativeSelectOption>{employers.map((item) => <NativeSelectOption key={item} value={item}>{item}</NativeSelectOption>)}</NativeSelect></div>
            <div><label className="sr-only" htmlFor="practice-difficulty">Difficulty</label><NativeSelect id="practice-difficulty" className="w-full" value={difficulty} onChange={(event) => setDifficulty(event.target.value)}><NativeSelectOption value="all">All difficulties</NativeSelectOption>{[1, 2, 3, 4, 5].map((level) => <NativeSelectOption key={level} value={level}>Difficulty {level}</NativeSelectOption>)}</NativeSelect></div>
            <div><label className="sr-only" htmlFor="practice-sort">Sort problems</label><NativeSelect id="practice-sort" className="w-full" value={sortBy} onChange={(event) => setSortBy(event.target.value)}><NativeSelectOption value="sequence">Curriculum order</NativeSelectOption><NativeSelectOption value="difficulty-asc">Difficulty: low to high</NativeSelectOption><NativeSelectOption value="difficulty-desc">Difficulty: high to low</NativeSelectOption><NativeSelectOption value="title">Title: A to Z</NativeSelectOption></NativeSelect></div>
          </div>
